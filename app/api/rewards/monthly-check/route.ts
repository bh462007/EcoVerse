  ...
  try {
    ...
    if (!lastCheck || lastCheck.getMonth() !== currentDate.getMonth()) {
      ...
      if (monthlyBonus) {
        ...
        user.confirmedPoints =
          (user.confirmedPoints || 0) + monthlyBonus.points;
        user.rewardPoints =
          (user.confirmedPoints || 0) + (user.unconfirmedPoints || 0);
        user.totalPointsEarned =
          (user.totalPointsEarned || 0) + monthlyBonus.points;
...
export async function GET(req: Request) {
  ...
  try {
    ...
    const eligibleForBonus =
      !lastCheck || lastCheck.getMonth() !== currentDate.getMonth();
