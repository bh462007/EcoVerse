...
export function BadgeCard({ badge }: { badge: BadgeProps }) {
  ...
  return (
    <Card className={`relative overflow-hidden transition-all duration-300 ${badge.isEarned ? 'bg-gradient-to-br from-green-50 to-emerald-100 border-green-200 shadow-md hover:shadow-lg hover:-translate-y-1' : 'bg-gray-50 border-gray-200 grayscale-[0.5] opacity-90'}`}>
      <CardContent className="p-6 flex flex-col items-center text-center h-full">
        ...
        {/* Icon */}
        <div className={`text-5xl mt-6 mb-4 transition-transform duration-300 ${badge.isEarned ? 'scale-110 drop-shadow-md' : 'opacity-60'}`}>
          {badge.icon}
        </div>

        {/* Info */}
        <h3 className={`text-lg font-bold mb-2 ${badge.isEarned ? 'text-green-900' : 'text-gray-700'}`}>
          {badge.name}
        </h3>
          ...
          {badge.isEarned ? (
            ...
            <div className="space-y-2">
              <div className="flex justify-between text-xs text-gray-500 font-medium">
                ...
                <span>{badge.currentValue} / {badge.maxValue}</span>
              </div>
